...
def compute_metrics(pred_actions: np.ndarray, gt_actions: np.ndarray) -> dict[str, float]:
...
def evaluate_actions(all_actions: list[dict]) -> dict[str, t.Any]:
    ...
    for traj_data in all_actions:
        ...
        first_call_time = traj_data.get('first_call_time', inference_times[0] if len(inference_times) > 0 else 0)
        ...
        results['mean_inference_time_excl_first'] = np.mean(inference_times[1:]) if len(inference_times) > 1 else 0
        ...
        print(f"  Action magnitude - Pred: {results['pred_mag']:.6f}, GT: {results['gt_mag']:.6f}, Ratio: {results['mag_ratio']:.6f}")
        
        # Print timing metrics with first call highlighted
        print(f"  Timing - First call: {first_call_time*1000:.2f}ms")
        print(f"  Timing - Mean (excl. first): {results['mean_inference_time_excl_first']*1000:.2f}ms")
        print(f"  Timing - Mean (all calls): {results['mean_inference_time']*1000:.2f}ms")
        print(f"  Timing - Median: {results['median_inference_time']*1000:.2f}ms")
        print(f"  Timing - Min: {results['min_inference_time']*1000:.2f}ms, Max: {results['max_inference_time']*1000:.2f}ms")
        print(f"  Timing - Std: {results['std_inference_time']*1000:.2f}ms, Total: {results['total_inference_time']:.2f}s")
    ...
    avg_metrics = {k: np.mean([r[k] for r in all_results]) for k in all_results[0] if k not in ['traj_idx', 'num_actions']}
...
def analyze_saved_results(save_dir: str = "results",
    ...
    try:
        ...
        if timestamp and host and port:
            ...
            matching_files = [f for f in files if timestamp in f and model_name in f and 
                             f.startswith('actions_') and f.endswith('.pkl')]
            if not matching_files:
                print(f"No files found with timestamp {timestamp} and model {model_name}")
                return
            ...
            files = os.listdir(save_dir)
            matching_files = [f for f in files if timestamp in f and f.startswith('actions_') and f.endswith('.pkl')]
            if not matching_files:
            ...
            files = os.listdir(save_dir)
            action_files = [f for f in files if model_name in f and f.startswith('actions_') and f.endswith('.pkl')]
            ...
            files = os.listdir(save_dir)
            action_files = [f for f in files if f.startswith('actions_') and f.endswith('.pkl')]
        ...
        pred_mags = [np.mean(np.linalg.norm(traj['pred_actions'], axis=1)) for traj in all_actions]
        gt_mags = [np.mean(np.linalg.norm(traj['gt_actions'], axis=1)) for traj in all_actions]
        
        avg_pred_mag = np.mean(pred_mags)
        avg_gt_mag = np.mean(gt_mags)
        
        print("\nAction Magnitude Analysis:")
        print(f"Average action magnitude - Predicted: {avg_pred_mag:.6f}, Ground Truth: {avg_gt_mag:.6f}")
        print(f"Ratio (Pred/GT): {avg_pred_mag/avg_gt_mag:.6f}")
        
        if avg_pred_mag < 0.1 * avg_gt_mag:
            print("WARNING: Predicted actions are much smaller than ground truth actions")
        elif avg_pred_mag > 2.0 * avg_gt_mag:
            print("WARNING: Predicted actions are much larger than ground truth actions")
